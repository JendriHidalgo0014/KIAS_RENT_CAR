...
interface UsuarioDao {
    ...
    @Query("SELECT * FROM usuarios WHERE remoteId = :remoteId")
    suspend fun getByRemoteId(remoteId: Int): UsuarioEntity?
    ...
    @Query("UPDATE usuarios SET isLoggedIn = 1 WHERE id = :id")
    suspend fun setLoggedIn(id: String)

    @Query("DELETE FROM usuarios WHERE id = :id")
    suspend fun deleteById(id: String)

    @Query("DELETE FROM usuarios WHERE remoteId = :remoteId")
